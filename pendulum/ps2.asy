...
// spring
path spring1 = (-1.2, by_c);
for(int i=0; i < 11; ++i) {
    spring1 = spring1 -- (-1.2+0.3*i/11, by_c+bh*(-1)^i);
}
path spring2 = (bx_2, by_c);
for(int i=0; i < 11; ++i) {
    spring1 = spring1 -- (bx_2+0.7*i/11, by_c+bh*(-1)^i);
}

draw(spring1, linewidth(0.7));
label("$k$", (bx01-0.1,by_c), align = 2S+W);
...
draw((-1.2, 0.1) -- (-1.2, -0.3), rod_style);
draw((1.2, 0.1) -- (1.2, -0.3), rod_style);
...
path base1 = circle((px0_1, py0_1), 0.5*bh); 
path base2 = circle((px0_2, py0_2), 0.5*bh); 
...
fill(circle((bx_c, by_c), 0.5*bh), purple);
label("$X$", (bx_c, by_c + bh), align = N, Fill(white));
label("$M$", (bx_c, by_c - bh), align = SW, purple);
draw(Label("$a$", Fill(white)), (bx_c, by_c - 2.2 * bh)--(px0_2, by_c - 2.2 * bh), red, Bars, Arrows, align=S);
