...
path background = box(-(half_width,half_height),(half_width + spacing + 2half_height,half_height));
path ellipse_ = ellipse((0,0),ellipse_a,ellipse_b);
path velocity_m = velocity_start -- impact;
path velocity_line = impact -- impact + speed*(cos(angle_theta),sin(angle_theta))*2.5;
...
label("$\vec{v}$", new_origin+velocity_shifted_scaled, align=N+W, blue);
draw(new_origin -- new_origin + normal_shifted_scaled, arrow=Arrow(SimpleHead), red);
label("$\vec{n}$", new_origin + normal_shifted_scaled, align=N, red);
draw(new_origin -- new_origin + vec_to_center_shifted_scaled, arrow=Arrow(SimpleHead), purple);
label("$\vec{R}$", new_origin+vec_to_center_shifted_scaled, align=S+E, purple);